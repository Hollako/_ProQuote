# ProQuote â€” BoQ pricing & quotation engine
...
| `ingest.py` | Folder-wide Excel scanner â†’ catalogue + project history |
...
`py ingest.py` is **idempotent** â€” it re-reads every workbook and refreshes
...
- Pricing chain (validated against the historical sheets, 99.9% match):
  - `Unit Cost $ = Ex Unit Cost Ã- 1.3`  (landed cost; manual if no Ex cost)
  - `U. Price $  = ROUNDUP(Unit Cost Ã- Margin, 0)`  (**Margin Ã-** is per-line input)
  - `U. Price SAR = ROUNDUP(U. Price $ Ã- 3.75 â†’ next multiple of 10)`
  - `Total Cost = Qty Ã- Unit Cost`; `T. Price = Qty Ã- U. Price`;
    `T. Price SAR = Qty Ã- U. Price SAR`
  - Set **Margin Ã- = 0** on a line to type `U. Price $` manually (e.g. services).
- USDâ†’SAR peg = **3.75**; VAT = **15%**. Constants live at the top of `calc.py`
  (`SAR_PER_USD`, `COST_BUFFER`, `SAR_ROUND_TO`).
- Markup factor = `Total Selling SAR Ã· (Total Cost USD Ã- 3.75)` (â‰ˆ1.69 headline).
...
rebrand, replace that PNG with one of similar aspect ratio â€” the header scales
